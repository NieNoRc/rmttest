...
class PartialTrainableBERTold(nn.Module):
    def __init__(self,modelpath:str,flag_layer_num:int=11,num_labels:int=2,dropout_rate:float=0.1,feature_layer:int=2,boost_factor:float=2.0): 
        super().__init__()
        ...
        self.classifier = nn.Sequential(nn.Linear((self.hugginBERT.config.hidden_size)*2*feature_layer, (self.hugginBERT.config.hidden_size)*2*feature_layer),
                                        nn.ReLU(),
                                        nn.Dropout(p=dropout_rate),
                                        nn.Linear((self.hugginBERT.config.hidden_size)*2*feature_layer, num_labels),
                                        )
    ...
    def forward(self,bertinput,supportinput:list,insert_support_flag:bool=True):
        ...
        features=[bertoutput.hidden_states[12-i] for i in range(0,self.feature_layer*2)]
...
class PartialTrainableBERT(nn.Module):
    ...
    def forward(self,bertinput,supportinput:list,insert_support_flag:bool=True):
        ...
        inplace_features=[bertoutput.hidden_states[12-i] for i in range(self.feature_layer,self.feature_layer*2)]
        support_info=self.insert_support_info(bertoutdup=inplace_features,supportinput=supportinput,insert_support_flag=insert_support_flag)
            ...
            for i in entry['insert_pos'][0].tolist():
                for j in range(0,self.feature_layer):
                    ...
                    for k in entry['support_wp_ids'][0].tolist():
                        bertoutdup[j][0][i+cnt]=self.boost_factor*entryoutput.hidden_states[12-j][0][k]
                        cnt+=1
        return bertoutdup
